...
export async function applyPaymentWebhook(env, event) {
  ...
  if (event.type === 'adjustment') {
    if (!/^adj_[a-z0-9]{26}$/.test(event.adjustmentId || '')) throw new Error('PADDLE_ADJUSTMENT_ID_INVALID');
    if (!/^txn_[a-z0-9]{26}$/.test(event.providerPaymentId || '')) throw new Error('PADDLE_TRANSACTION_ID_INVALID');
    if (event.adjustmentAction !== 'refund') {
      await env.DB.prepare('INSERT INTO payment_webhook_events (event_id,event_type,processed_at) VALUES (?,?,?)').bind(event.eventId, event.eventType, now()).run();
      return { ignored: true, reason: 'NON_REFUND_ADJUSTMENT' };
    }
    if (!['PENDING_APPROVAL', 'APPROVED', 'REJECTED', 'REVERSED'].includes(event.adjustmentStatus)) throw new Error('PADDLE_ADJUSTMENT_STATUS_INVALID');
    const refund = await env.DB.prepare("SELECT refund_requests.*,payments.user_id FROM refund_requests JOIN payments ON payments.id=refund_requests.payment_id WHERE refund_requests.provider_adjustment_id=? OR (refund_requests.provider_transaction_id=? AND refund_requests.provider_adjustment_id IS NULL)").bind(event.adjustmentId, event.providerPaymentId).first();
    if (!refund) {
      await env.DB.prepare('INSERT INTO payment_webhook_events (event_id,event_type,processed_at) VALUES (?,?,?)').bind(event.eventId, event.eventType, now()).run();
      return { ignored: true, reason: 'REFUND_NOT_LINKED' };
    }
    const statements = [
      env.DB.prepare('UPDATE refund_requests SET provider_adjustment_id=?,status=?,updated_at=? WHERE id=?').bind(event.adjustmentId, event.adjustmentStatus, now(), refund.id),
      env.DB.prepare('INSERT INTO payment_webhook_events (event_id,event_type,processed_at) VALUES (?,?,?)').bind(event.eventId, event.eventType, now()),
    ];
    if (event.adjustmentStatus === 'APPROVED') {
      statements.push(env.DB.prepare("UPDATE payments SET status='REFUNDED' WHERE id=? AND status='PAID'").bind(refund.payment_id));
      statements.push(env.DB.prepare("UPDATE subscriptions SET status='CANCELED',cancel_at_period_end=0,pending_plan_id=NULL,pending_action=NULL,pending_action_at=NULL,updated_at=? WHERE user_id=? AND cancel_at_period_end=1").bind(now(), refund.user_id));
    }
    await env.DB.batch(statements);
    return { refundUpdated: true, status: event.adjustmentStatus, userId: refund.user_id };
  }
  ...
  if (event.type === 'subscription') {
    ...
    try {
      await env.DB.batch([
        env.DB.prepare('UPDATE subscriptions SET plan_id=?,status=?,provider_price_id=?,current_period_start=?,current_period_end=?,cancel_at_period_end=?,pending_plan_id=NULL,pending_action=NULL,pending_action_at=NULL,updated_at=?,provider_event_at=? WHERE user_id=?').bind(planId, status, priceId, periodStart, periodEnd, active && event.cancelAtPeriodEnd ? 1 : 0, now(), event.occurredAt || now(), subscription.user_id),
        env.DB.prepare('INSERT INTO payment_webhook_events (event_id,event_type,processed_at) VALUES (?,?,?)').bind(event.eventId, event.eventType, now()),
      ]);
