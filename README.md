...
Cloudflare 운영 환경은 Toss Payments provider를 사용합니다. `TOSS_MODE=test`에서는 실제 청구 없이 전체 결제 흐름을 검증하고, 전자결제 계약 후 라이브 키와 `TOSS_MODE=live`를 함께 적용하면 실결제가 활성화됩니다. 카드번호와 CVC는 Toss Payments만 처리하며 이 서버에 저장하지 않습니다.
...
- 결제 금액은 브라우저 값이 아닌 서버에 저장된 요금제 금액으로 승인합니다.
